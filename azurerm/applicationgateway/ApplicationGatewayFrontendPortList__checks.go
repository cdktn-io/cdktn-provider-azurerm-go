...
func (a *jsiiProxy_ApplicationGatewayFrontendPortList) validateResolveParameters(context cdktf.IResolveContext) error {
	if context == nil {
...
func (j *jsiiProxy_ApplicationGatewayFrontendPortList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*ApplicationGatewayFrontendPort:
		val := val.(*[]*ApplicationGatewayFrontendPort)
	...
	case []*ApplicationGatewayFrontendPort:
		val_ := val.([]*ApplicationGatewayFrontendPort)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*ApplicationGatewayFrontendPort; received %#v (a %T)", val, val)
		}
	}
