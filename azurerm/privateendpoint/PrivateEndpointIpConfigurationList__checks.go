...
func (p *jsiiProxy_PrivateEndpointIpConfigurationList) validateResolveParameters(context cdktf.IResolveContext) error {
	if context == nil {
...
func (j *jsiiProxy_PrivateEndpointIpConfigurationList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*PrivateEndpointIpConfiguration:
		val := val.(*[]*PrivateEndpointIpConfiguration)
	...
	case []*PrivateEndpointIpConfiguration:
		val_ := val.([]*PrivateEndpointIpConfiguration)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*PrivateEndpointIpConfiguration; received %#v (a %T)", val, val)
		}
	}
