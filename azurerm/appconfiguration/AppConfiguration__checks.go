...
func (a *jsiiProxy_AppConfiguration) validatePutReplicaParameters(value interface{}) error {
	...
	case *[]*AppConfigurationReplica:
		value := value.(*[]*AppConfigurationReplica)
	...
	case []*AppConfigurationReplica:
		value_ := value.([]*AppConfigurationReplica)
		value := &value_
		for idx_cd4240, v := range *value {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter value[%#v]", idx_cd4240) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(value) {
			return fmt.Errorf("parameter value must be one of the allowed types: cdktn.IResolvable, *[]*AppConfigurationReplica; received %#v (a %T)", value, value)
		}
	}
...
func (j *jsiiProxy_AppConfiguration) validateSetLifecycleParameters(val *cdktn.TerraformResourceLifecycle) error {
	if err := _jsii_.ValidateStruct(val, func() string { return "parameter val" }); err != nil {
		return err
	}
